...
var $author$project$Marks$isSpace = function(c) {
	return _Utils_eq(c, _Utils_chr(' ')) || (_Utils_eq(c, _Utils_chr('\n')) || (_Utils_eq(c, _Utils_chr('\t')) || _Utils_eq(c, _Utils_chr('\r'))));
};
var $author$project$Marks$skipSpace$ = function(chars, at) {
	skipSpace:
	while (true) {
		var _v0 = A2($gren_lang$core$Array$get, at, chars);
		if (_v0.$ === 'Just') {
			var c = _v0.a;
			if ($author$project$Marks$isSpace(c)) {
				var $temp$chars = chars,
				$temp$at = at + 1;
				chars = $temp$chars;
				at = $temp$at;
				continue skipSpace;
...
var $author$project$Marks$skipSpace = F2($author$project$Marks$skipSpace$);
var $author$project$Marks$matchAt$ = function(document, needle, si, qi) {
	matchAt:
	while (true) {
		var _v0 = A2($gren_lang$core$Array$get, qi, needle);
		if (_v0.$ === 'Nothing') {
			return $gren_lang$core$Maybe$Just(si);
		} else {
			var nc = _v0.a;
			var _v1 = A2($gren_lang$core$Array$get, si, document);
			if (_v1.$ === 'Nothing') {
				return $gren_lang$core$Maybe$Nothing;
			} else {
				var dc = _v1.a;
				if ($author$project$Marks$isSpace(dc) && $author$project$Marks$isSpace(nc)) {
					var $temp$document = document,
					$temp$needle = needle,
					$temp$si = $author$project$Marks$skipSpace$(document, si),
					$temp$qi = $author$project$Marks$skipSpace$(needle, qi);
...
var $author$project$Marks$matchAt = F4($author$project$Marks$matchAt$);
var $author$project$Marks$searchFrom$ = function(document, needle, at) {
	searchFrom:
	while (true) {
		if (_Utils_cmp(at, $gren_lang$core$Array$length(document)) > -1) {
			return $gren_lang$core$Maybe$Nothing;
		} else {
			var _v0 = $author$project$Marks$matchAt$(document, needle, at, 0);
			if (_v0.$ === 'Just') {
				var end = _v0.a;
				return $gren_lang$core$Maybe$Just({ end: end, start: at });
			} else {
				var $temp$document = document,
				$temp$needle = needle,
				$temp$at = at + 1;
...
var $author$project$Marks$searchFrom = F3($author$project$Marks$searchFrom$);
...
var $author$project$Marks$find$ = function(document, quote) {
	var needle = $gren_lang$core$String$toArray($gren_lang$core$String$trim(quote));
	return $gren_lang$core$Array$isEmpty(needle) ? $gren_lang$core$Maybe$Nothing : $author$project$Marks$searchFrom$(document, needle, 0);
};
var $author$project$Marks$find = F2($author$project$Marks$find$);
...
var $author$project$Check$document = 'gren-tvision documentation' + ('Turbo Vision terminal UIs, as the Elm architecture. Four documents, and they answer different questions.' + ('Turbo Vision, from Gren -- what you are programming against.' + 'Driving a C or C++ library from Gren -- the general problem, of which this binding is one instance. The three doors out of a Gren program and which one is yours, and never the CLIPBOARD selection.'));
var $author$project$Marks$plainText = function(text) {
	return $gren_lang$core$String$toArray(text);
};
var $author$project$Check$plain = $author$project$Marks$plainText($author$project$Check$document);
var $gren_lang$core$String$slice = _String_slice;
var $author$project$Check$slice = function(range) {
	return A3($gren_lang$core$String$slice, range.start, range.end, $author$project$Check$document);
};
...
var $author$project$Marks$cut$ = function(base, from, to, text) {
	return A3($gren_lang$core$String$slice, from - base, to - base, text);
};
var $author$project$Marks$cut = F4($author$project$Marks$cut$);
...
var $author$project$Marks$pieces$ = function(ranges, base, text, stop, at, acc) {
	if (_Utils_cmp(at, stop) > -1) {
		return acc;
	} else {
		var _v0 = A2($gren_lang$core$Array$findFirst, function(range) {
				return (_Utils_cmp(range.end, at) > 0) && (_Utils_cmp(range.start, stop) < 0);
			}, ranges);
		if (_v0.$ === 'Nothing') {
			return $gren_lang$core$Array$pushLast$({ marked: false, text: $author$project$Marks$cut$(base, at, stop, text) }, acc);
		} else {
			var found = _v0.a;
			if (_Utils_cmp(found.value.start, at) > 0) {
				return $author$project$Marks$pieces$(ranges, base, text, stop, found.value.start, $gren_lang$core$Array$pushLast$({ marked: false, text: $author$project$Marks$cut$(base, at, found.value.start, text) }, acc));
			} else {
				var upTo = $gren_lang$core$Basics$min$(found.value.end, stop);
				return $author$project$Marks$pieces$(ranges, base, text, stop, upTo, $gren_lang$core$Array$pushLast$({ marked: true, text: $author$project$Marks$cut$(base, at, upTo, text) }, acc));
			}
		}
	}
};
var $author$project$Marks$pieces = F6($author$project$Marks$pieces$);
var $author$project$Marks$split$ = function(ranges, base, text) {
	var stop = base + $gren_lang$core$String$count(text);
	return $gren_lang$core$String$isEmpty(text) ? [  ] : $author$project$Marks$pieces$(ranges, base, text, stop, base, [  ]);
};
var $author$project$Marks$split = F3($author$project$Marks$split$);
var $author$project$Check$checks = function () {
	var wrapped = $author$project$Marks$find$($author$project$Check$plain, 'The three doors out of a\nGren program and which one is yours,');
	var inline = $author$project$Marks$find$($author$project$Check$plain, 'Turbo Vision, from Gren');
	var codeSpan = $author$project$Marks$find$($author$project$Check$plain, 'never the CLIPBOARD selection');
	return [ { name: 'a quote across a wrap is found', ok: !_Utils_eq(wrapped, $gren_lang$core$Maybe$Nothing) }, { name: 'a quote across a wrap covers exactly the passage', ok: _Utils_eq($gren_lang$core$Maybe$map$($author$project$Check$slice, wrapped), $gren_lang$core$Maybe$Just('The three doors out of a Gren program and which one is yours,')) }, { name: 'a quote inside link and bold markup is found', ok: _Utils_eq($gren_lang$core$Maybe$map$($author$project$Check$slice, inline), $gren_lang$core$Maybe$Just('Turbo Vision, from Gren')) }, { name: 'a quote across a code span is found', ok: _Utils_eq($gren_lang$core$Maybe$map$($author$project$Check$slice, codeSpan), $gren_lang$core$Maybe$Just('never the CLIPBOARD selection')) }, { name: 'a deleted passage is not found', ok: _Utils_eq($author$project$Marks$find$($author$project$Check$plain, 'a sentence that is no longer in the document'), $gren_lang$core$Maybe$Nothing) }, { name: 'a blank quote is not a passage', ok: _Utils_eq($author$project$Marks$find$($author$project$Check$plain, '   '), $gren_lang$core$Maybe$Nothing) }, { name: 'folding never inserts whitespace that is not there', ok: _Utils_eq($author$project$Marks$find$($author$project$Check$plain, 'documentation Turbo Vision terminal'), $gren_lang$core$Maybe$Nothing) }, { name: 'folding never joins two words', ok: _Utils_eq($author$project$Marks$find$($author$project$Marks$plainText('one two three'), 'onetwo'), $gren_lang$core$Maybe$Nothing) }, { name: 'a quote is found from the front, not the middle of a word', ok: _Utils_eq($gren_lang$core$Maybe$map$(function ($) {
				return $.start;
			}, $author$project$Marks$find$($author$project$Check$plain, 'Turbo Vision terminal')), $gren_lang$core$Maybe$Just(26)) }, { name: 'a range wholly inside one run', ok: _Utils_eq($author$project$Marks$split$([ { end: 5, start: 2 } ], 0, 'abcdefg'), [ { marked: false, text: 'ab' }, { marked: true, text: 'cde' }, { marked: false, text: 'fg' } ]) }, { name: 'a range that began in an earlier run', ok: _Utils_eq($author$project$Marks$split$([ { end: 12, start: 0 } ], 10, 'abcdefg'), [ { marked: true, text: 'ab' }, { marked: false, text: 'cdefg' } ]) }, { name: 'a range that continues into a later run', ok: _Utils_eq($author$project$Marks$split$([ { end: 40, start: 12 } ], 10, 'abcdefg'), [ { marked: false, text: 'ab' }, { marked: true, text: 'cdefg' } ]) }, { name: 'a range that covers the whole run', ok: _Utils_eq($author$project$Marks$split$([ { end: 100, start: 0 } ], 10, 'abcdefg'), [ { marked: true, text: 'abcdefg' } ]) }, { name: 'a run no range touches', ok: _Utils_eq($author$project$Marks$split$([ { end: 60, start: 50 } ], 10, 'abcdefg'), [ { marked: false, text: 'abcdefg' } ]) }, { name: 'two ranges in one run', ok: _Utils_eq($author$project$Marks$split$([ { end: 2, start: 1 }, { end: 6, start: 4 } ], 0, 'abcdefg'), [ { marked: false, text: 'a' }, { marked: true, text: 'b' }, { marked: false, text: 'cd' }, { marked: true, text: 'ef' }, { marked: false, text: 'g' } ]) }, { name: 'an empty run yields no pieces', ok: _Utils_eq($author$project$Marks$split$([ { end: 5, start: 0 } ], 0, ''), [  ]) }, { name: 'the pieces of a run always rebuild it', ok: A3($gren_lang$core$Array$foldl, F2(function(piece, acc) {
				return _Utils_ap(acc, piece.text);
			}), '', $author$project$Marks$split$([ { end: 14, start: 11 } ], 10, 'abcdefg')) === 'abcdefg' }, { name: 'a selection spanning two blocks is not marked', ok: _Utils_eq($author$project$Marks$find$($author$project$Check$plain, 'documentation\nTurbo Vision terminal'), $gren_lang$core$Maybe$Nothing) } ];
}();
...
var $author$project$Check$main = $gren_lang$node$Node$defineSimpleProgram(function(env) {
		var line = function(check) {
			return _Utils_ap(check.ok ? '  ok    ' : '  FAIL  ', check.name);
		};
		var failures = A2($gren_lang$core$Array$keepIf, function(check) {
				return !check.ok;
			}, $author$project$Check$checks);
		var report = A2($gren_lang$core$String$join, '\n', A2($gren_lang$core$Array$map, line, $author$project$Check$checks)) + ('\n\n' + ($gren_lang$core$String$fromInt($gren_lang$core$Array$length($author$project$Check$checks)) + (' checks, ' + ($gren_lang$core$String$fromInt($gren_lang$core$Array$length(failures)) + ' failed'))));
		return $gren_lang$node$Node$endSimpleProgram(A2($gren_lang$core$Task$onError, function(_v1) {
					return $gren_lang$core$Task$succeed({  });
				}, A2($gren_lang$core$Task$andThen, function(_v0) {
						return $gren_lang$node$Node$setExitCode($gren_lang$core$Array$length(failures));
					}, $gren_lang$core$Stream$writeLineAsBytes$(report, env.stdout))));
	});
_Platform_export({'Check':{'init':$author$project$Check$main($gren_lang$core$Json$Decode$succeed({  }))}});}(this.module ? this.module.exports : this));